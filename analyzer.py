...
print(f'''Dla produktu o kodzie {product_code} pobranych zostało {stats["opinions_count"]} opinii. Dla {stats["pros_count"]} opinii podana została lista zalet produktu, a dla {stats["cons_count"]}
opinii lista jego wad. Średnia ocena produktu wynosi {stats["average_score"]:.2f}.''')
...
recommendation = opinions.recommendation.value_counts(dropna=False) #.reindex(list(np.arange(0, 5.5, 0.5)), fill_value=0)
